...
class _ProfileInformationWidgetState extends State<ProfileInformationWidget> {
  ...
  Future<void> _onProfileImageUpdated(String imagePath) async {
    ...
    final b64Image = base64.encode(imageBytes);
    final token = await TokenStorage.getToken();
    await _authService.updateUserPicture(
      PictureRequest(image: b64Image),
      token,
    );
  }
                  ...
                  Container(
                    ...
                    child: Center(
                      child: ProfileCircleAvatar(
                        user: user!,
                        onPictureChanged: (imagePath) async {
                          try {
                            await _onProfileImageUpdated(imagePath);
